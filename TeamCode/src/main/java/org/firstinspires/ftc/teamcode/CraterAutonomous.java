...
@Autonomous(group = "5961", name = "Crater Autonomous")
...
//        lift.setTargetPosition((int)(COUNTS_PER_MOTOR_REV*20/60*5.7));
        ...
        sleep(500);
        double[] goldAtPos1 = lookForMineral(MineralType.Gold, 20, 80);
        // middle
        // x1 122 x2 92
        //x1 112 x2 77
        //x1 104 x2 86
...
//        setMotorRunMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
            sleep(300);
            double[] goldPos = lookForMineral(MineralType.Gold,0,200);
...
//            distance is in cm and always positive
    ...
    private double[] lookForMineral(MineralType mineralType, double minumumHorizDistance, int maxY){
...
//        sleep(300);
        ...
        for (Pair<double[], double[]> pair: pairs){
            if (pair.first[1] < maxY && pair.second[1] > maxY && pair.first[2] > 600 && pair.second[2] > 600 && pair.first[2] < 4000 && pair.second[2] < 4000 &&
                    isAboutEqual(pair.first[2],pair.second[2], 3000) &&
                    isAboutEqual(pair.first[1], pair.second[1], 10) && pair.first[0] - minumumHorizDistance > pair.second[0]) {
